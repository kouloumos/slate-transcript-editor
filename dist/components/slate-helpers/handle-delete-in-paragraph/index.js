...
function handleDeleteInParagraph({
  ...
  if (isSameBlock(anchorPath, focusPath)) {
    if (isBeginningOftheBlock(anchorOffset, focusOffset)) {
      ...
      const currentBlockChapter = currentBlockNode.chapter; // if the word have changed. then re-align paragraph before splitting.
      ...
      const newWords = [...previousBlockWordsList, ...currentBlockWordsList]; // when merging two blocks, priority is given to the chapter of the first block
